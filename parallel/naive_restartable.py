"""
Similar to naive.py, however has a callback which saves data so that many iterations can be run over multiple sessions.
NOTE: There is no phi dependence in this module.
"""
...
def Optimise(numItersPerCore, toSave = False):
    ...
    if rank == 0:
        ...
        optimizer = Optimizer(dimensions = problemBounds, base_estimator = "ET", n_initial_points = int(np.ceil(1e10 * numCores/10)), acq_func = "LCB")
        try: 
            prevRes = load("naive_model_nophi.pkl")
            optimizer.tell(prevRes.Xi, prevRes.yi)
            print("Previous results loaded!")
        except:
            print("Issue loading previous results, starting from scratch.")
    ...
    if rank == 0:
        pbar.close()
        print("\nGlobal optimum region found, saving optimisation and starting local optimisation at best point.")
        dump(optimizer, "naive_model_nophi.pkl")
        ...
        #print("\nOptimisation for three filters (naive + extra polariser, no phi) complete. Best FoM: {}, number of evaluations: {}, best parameters: {}".format(abs(finResult.fun), finResult.nfev + len(optimizer.yi), finResult.x))
        ...
        if toSave:
            ...
            joblib.dump((optimizer.models[-1], resultDataframe, bestIndex), "shap_data_naive_extra_pol_nophi.pkl")
            print("Data saved to shap_data_naive_extra_pol.pkl_nophi.")

    return

if __name__ == "__main__":
    # Run the optimisation.
    Optimise(200, toSave = False)
    Optimise(200, toSave = False)
    Optimise(200, toSave = False)
    Optimise(200, toSave = False)
    Optimise(200, toSave = False)
