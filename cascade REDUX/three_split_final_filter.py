...
"""
Exploits the similarity of the two output fields to 'easily' combine the beams.
"""
...
def ThreeFilter(inputParams):
    ...
    waveplateAng = np.deg2rad(inputParams[8])
    ...
    pbsMatrixAccept = np.matrix([[np.cos(np.deg2rad(inputParams[9]))**2, np.sin(np.deg2rad(inputParams[9]))*np.cos(np.deg2rad(inputParams[9])), 0],
    ...
    # Pass combined field through another waveplate.
    halfWaveplate2 = np.matrix([[np.cos(np.deg2rad(inputParams[10])), np.sin(np.deg2rad(inputParams[10])), 0],
								[np.sin(np.deg2rad(inputParams[10])), -1 * np.cos(np.deg2rad(inputParams[10])), 0],
                                [0, 0, 1]])

    combinedField = np.array(halfWaveplate2 * combinedField[:])
...
def OptimiseThreeFilter(numItersPerCore, toSave = False):
    ...
    if rank == 0:
        # Set up the problem.
        problemBounds = [(10., 1300.), (40., 230.), (0., 90.), (0., 90.), (10., 1300.), (40., 230.), (0., 90.), (0., 90.), (0., 180.), (0., 180.), (0., 180.)]
...
if __name__ == "__main__":
    ...
    OptimiseThreeFilter(100, toSave = False)