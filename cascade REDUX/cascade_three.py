...
def ThreeFilterFitness(inputParams):
    ...
    try:
        # There may at times be issues with ElecSus, such as when NaN is entered as a variable.
        [outputEFin] = elecsus.calculate(globalDetuning, outputESecond, filterFinParams, outputs = ["E_out"])
    except:
...
def Optimise(numItersPerCore, toSave = False):
    ...
    if rank == 0:
        ...
        print("\nOptimisation for three filters (three cascaded filters) complete. Best FoM: {}, number of evaluations: {}, best parameters: {}".format(abs(finResult.fun), finResult.nfev + len(optimizer.yi), finResult.x))
        ...
        if toSave:
            ...
            joblib.dump((optimizer.models[-1], resultDataframe, bestIndex), "shap_data_three_cascade.pkl")
            print("Data saved to shap_data_three_cascade.pkl.")
