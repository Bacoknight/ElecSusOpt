...
def ThreeFilter(inputParams):
    ...
    #combinedField = outputEL + outputER
    # print("Combined field at peak: {}".format(np.linalg.norm(combinedField[:, peakArg])))
...
def OptimiseThreeFilter(numItersPerCore, toSave = False):
    ...
    if rank == 0:
        ...
        print("\nOptimisation for three filters (fixed rotation) complete. Best FoM: {}, number of evaluations: {}, best parameters: {}".format(abs(finResult.fun), finResult.nfev + len(optimizer.yi), finResult.x))
        ...
        if toSave:
            ...
            joblib.dump((optimizer.models[-1], resultDataframe, bestIndex), "shap_data_rotator.pkl")
            print("Data saved to shap_data_rotator.pkl.")
...
if __name__ == "__main__":
    ...
    OptimiseThreeFilter(2000, toSave = False)